...
void CVideo::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EXPLORER3, m_screenc);
	DDX_Control(pDX, IDC_EXPLORER2, m_screenu);
}
...
BEGIN_EASYSIZE_MAP(CVideo)
	EASYSIZE(IDC_SCREEN,ES_BORDER,ES_BORDER,IDC_EXPLORER2,IDC_SLIDER1,0)
	EASYSIZE(IDC_SLIDER1, ES_BORDER,IDC_SCREEN,ES_BORDER,IDC_SPLITTER,0)
    EASYSIZE(IDC_SPLITTER,ES_BORDER,IDC_SLIDER1,ES_BORDER,IDC_EXPLORER3,0)
	EASYSIZE(IDC_EXPLORER3,ES_BORDER,IDC_SPLITTER,ES_BORDER, IDC_EXPLORER1,0)
	EASYSIZE(IDC_EXPLORER1,ES_BORDER,IDC_EXPLORER3,ES_BORDER,ES_BORDER,0)
	EASYSIZE(IDC_EXPLORER2,IDC_SCREEN ,ES_BORDER,ES_BORDER,IDC_SLIDER1,0);
END_EASYSIZE_MAP
//
...
BOOL CVideo::OnInitDialog()
{
	...
	// get panel urls
	//
	TCHAR Panel1[255],Panel2[255];
	wsprintf(sz,"Select * from panels where id=1");
	mysql_real_query(Log.myData,sz,sizeof(sz));
	Log.res = mysql_store_result( Log.myData );
	while ( Log.row = mysql_fetch_row(Log.res ) ) 
	{
		strcpy(Panel1,Log.row[1]);
	}
	wsprintf(Panel1,"%s?id=%s&uid=%s",Panel1,Pr->id,Log.User.user);
	wsprintf(sz,"Select * from panels where id=2");
	mysql_real_query(Log.myData,sz,sizeof(sz));
	Log.res = mysql_store_result( Log.myData );
	while ( Log.row = mysql_fetch_row(Log.res ) ) 
	{
		strcpy(Panel2,Log.row[1]);
	}
	wsprintf(Panel2,"%s%s&chat=is%20Online",Panel2,Log.User.user);
	
	m_screenc.Navigate(Path,NULL,NULL,NULL,NULL);
	m_screenu.Navigate(Panel1,NULL,NULL,NULL,NULL);
	m_screenc.Navigate(Panel2,NULL,NULL,NULL,NULL);
...
LONG CVideo::StartPlay(WPARAM wParam,LPARAM lParam)   // wparam = file size in mpbs, lparam=d/l sp in mbps
{
	...
	int nTotal =(int)(g_rtTotalTime / 10000000);
	//TCHAR tt[200];
	//wsprintf(tt,"tp->%d s, fs->%d, d/l s->%d",nTotal,_Fs,(int)lParam);
	//AfxMessageBox(tt,MB_OK);
	int _Sp = _Fs/nTotal;
	Play=1;
	if(_Sp>(int)lParam) /// making smooth buffering
	{
		tWait = ((_Fs/(int)lParam)-nTotal)-2;
...
BORDER-TOP-STYLE: solid;'>\");</SCRIPT>",tWait);
	...
	if (g_bAudioOnly)
	{
		...
		//CSplitterControl::ChangePos(&m_screenc,0,-dy);
...
void CVideo::DoResize(int delta)
{
	...
	CSplitterControl::ChangeHeight(&m_screenu, delta, CW_TOPALIGN);
	if(ProjectActive)CenterVideo();
	CSplitterControl::ChangePos(&m_screenc,0,delta);
	...
	if(pScrollBar->m_hWnd==m_Slider.m_hWnd)
	{
		...
		for(unsigned int i=0;i<PageCount;i++)
		{
			if((int)(Position/10)==(int)(Pages[i].Frame/10))
			{
				...
				m_explorer.Navigate(html,NULL,NULL,NULL,NULL);
				//m_screenc.Navigate(html,NULL,NULL,NULL,NULL);
				//m_screenu.Navigate(html,NULL,NULL,NULL,NULL);
			}
		}
