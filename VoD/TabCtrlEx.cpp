...
int CTabCtrlEx::AddPage (LPCTSTR pszTitle, int nPageID, CTabPage* pTabPage) {
...
int CTabCtrlEx::AddPage (LPCTSTR pszTitle, int nPageID, LPCTSTR pszTemplateName) {
	// Verify that the dialog template is compatible with CTabCtrlEx
...
int CTabCtrlEx::AddPage (LPCTSTR pszTitle, int nPageID, int nTemplateID) {
	return AddPage (pszTitle, nPageID, MAKEINTRESOURCE (nTemplateID));
}

BOOL CTabCtrlEx::RemovePage (int nIndex) {
    ...
    CTabPage* pDialog = tabDelete.pTabPage;
	if (pDialog != NULL) {
		pDialog->DestroyWindow ();	
		delete pDialog;
	}
...
int CTabCtrlEx::GetPageCount (void) {
	return GetItemCount ();
}


int CTabCtrlEx::GetPageID (int nIndex) {
	if (nIndex >= GetItemCount ())
		return -1;
...
int CTabCtrlEx::SetPageID (int nIndex, int nPageID) {
	if (nIndex >= GetItemCount ())
		return -1;
...
BOOL CTabCtrlEx::ActivatePage (int nIndex) 
{
	if (nIndex >= GetItemCount ())
		return FALSE;
...
int CTabCtrlEx::GetActivePage (void) {
	return GetCurSel ();
}

CWnd* CTabCtrlEx::GetPage (int nIndex) {
	if (nIndex >= GetItemCount ())
		return NULL;
...
int CTabCtrlEx::GetPageIndex (int nPageID) {
	int nCount = GetItemCount ();
	if (nCount == 0)
		return -1;
...
#ifdef _DEBUG
BOOL CTabCtrlEx::CheckDialogTemplate (LPCTSTR pszTemplateName) {
...
void CTabCtrlEx::ResizeDialog (int nIndex, int cx, int cy) {
	if (nIndex != -1) {
        TabDelete tabDelete = m_tabs[nIndex];
        CTabPage* pDialog = tabDelete.pTabPage;
...
BOOL CTabCtrlEx::OnInitPage (int /*nIndex*/, int /*nPageID*/) {
	// TODO: Override in derived class to initialise pages.
	return TRUE;
}

void CTabCtrlEx::OnActivatePage (int /*nIndex*/, int /*nPageID*/) {
	// TODO: Override in derived class to respond to page activations.
	extern int ProjectActive;
	if(ProjectActive) ActivatePage(1);
	
}

void CTabCtrlEx::OnDeactivatePage (int /*nIndex*/, int /*nPageID*/) {
	// TODO: Override in derived class to respond to page deactivations.
}

void CTabCtrlEx::OnDestroyPage (int nIndex, int nPageID) {
...
void CTabCtrlEx::OnSetFocus (CWnd* pOldWnd) {
...
void CTabCtrlEx::OnKillFocus (CWnd* pNewWnd) {
...
void CTabCtrlEx::OnDestroy (void) {
	int nCount = (int)m_tabs.GetSize ();

	// Destroy dialogs and delete CTabCtrlEx objects.
	if (nCount > 0) {
...
BOOL CTabCtrlEx::OnNotify (WPARAM wParam, LPARAM lParam, LRESULT* /*pResult*/) {
	// Forward WM_NOTIFY messages to the dialog's parent.
	return (BOOL)GetParent ()->SendMessage (WM_NOTIFY, wParam, lParam);
}

BOOL CTabCtrlEx::OnCmdMsg (UINT nID, int nCode, void* pExtra,
...
int CTabCtrlEx::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete) {
	// Add a page to the tab control.
	TC_ITEM item;
	item.mask = TCIF_TEXT;
